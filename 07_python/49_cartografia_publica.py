...
<head>
  ...
  <style>
    ...
    .leaflet-tooltip.alt-label { background:rgba(255,255,255,.95); border:1px solid #6b7280; border-radius:3px; color:#111827; font-size:11px; font-weight:700; line-height:1.1; padding:2px 4px; box-shadow:0 1px 2px rgba(0,0,0,.18); }
    .leaflet-tooltip.alt-label:before { display:none; }
...
function markerFor(feature, def) {
  ...
  // O código fica permanentemente visível: a leitura da alternativa não depende
  // de passar o cursor sobre o ponto, inclusive em capturas para o relatório.
  marker.bindTooltip(esc(p.codigo.replace('-PROPOSTO','')), { permanent:true, direction:'right', offset:[10,0], className:'alt-label', opacity:.96 });
  return marker;
}
